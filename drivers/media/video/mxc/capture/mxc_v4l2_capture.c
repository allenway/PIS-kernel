...
static int verify_preview(cam_data *cam, struct v4l2_window *win)
{
	...
	log_msg(LOG_DEBUG,"win->h(%d),win->w(%d)\n",win->w.height,win->w.width);
printk("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
	do {
...
static int mxc_v4l2_s_fmt(cam_data *cam, struct v4l2_format *f)
{
	...
	log_msg(LOG_DEBUG,"width = %d, height = %d\n",f->fmt.pix.width,f->fmt.pix.height);
	switch(f->fmt.pix.pixelformat){
		case V4L2_PIX_FMT_RGB32:
			log_msg(LOG_DEBUG,"set format V4L2_PIX_FMT_RGB32\n");
			break;
		case V4L2_PIX_FMT_YUYV:
			log_msg(LOG_DEBUG,"set format V4L2_PIX_FMT_YUYV\n");
			break;
		case V4L2_PIX_FMT_UYVY:
			log_msg(LOG_DEBUG,"set format V4L2_PIX_FMT_UYVY\n");
			break;
		case V4L2_PIX_FMT_VYUY:
			log_msg(LOG_DEBUG,"set format V4L2_PIX_FMT_VYUY\n");
			break;
		case V4L2_PIX_FMT_YUV420:
			log_msg(LOG_DEBUG,"set format V4L2_PIX_FMT_YUV420\n");
			break;
		case V4L2_PIX_FMT_YUV422P:
			log_msg(LOG_DEBUG,"set format V4L2_PIX_FMT_YUV422P\n");
			break;
		default:
			log_msg(LOG_DEBUG,"set format others\n");		
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		...
		if (strcmp(mxc_capture_inputs[cam->current_input].name,
			   "CSI MEM") == 0) {
			log_msg(LOG_DEBUG,"csi mem called\n");
#if 0		
...
#else
			cam->crop_current.width = f->fmt.pix.width = 640;
			cam->crop_current.height = f->fmt.pix.height = 480;
#endif
		}
		...
		if (cam->rotation >= IPU_ROTATE_90_RIGHT) {
			...
			log_msg(LOG_DEBUG,"rotation > IPU_ROTATE_90_RIGHT\n");
		} else {
			width = &(f->fmt.pix.width);
			height = &(f->fmt.pix.height);
			log_msg(LOG_DEBUG,"rotation < IPU_ROTATE_90_RIGHT\n");
		}
		log_msg(LOG_DEBUG,"w=%d h=%d fmt->w(%d),fmt->h(%d)\n",*width,*height,f->fmt.pix.width,f->fmt.pix.height);
		/* stride line limitation */
		*width -= *width % 8;
		*height -= *height % 8;
...
static int mxc_v4l2_g_ctrl(cam_data *cam, struct v4l2_control *c)
{
	...
	case V4L2_CID_HFLIP:
		...
		if (cam->rotation == IPU_ROTATE_HORIZ_FLIP)
			c->value = 1;
log_msg(LOG_DEBUG,"V4L2_CID_HFLIP,val=%d\n",c->value);
		break;
	case V4L2_CID_VFLIP:
		/* This is handled in the ipu. */
		if (cam->rotation == IPU_ROTATE_VERT_FLIP)
			c->value = 1;
log_msg(LOG_DEBUG,"V4L2_CID_VFLIP,val=%d\n",c->value);
		break;
	case V4L2_CID_MXC_ROT:
		/* This is handled in the ipu. */
		c->value = cam->rotation;
log_msg(LOG_DEBUG,"V4L2_CID_MXC_ROT,val=%d\n",c->value);
		break;
...
log_msg(LOG_DEBUG,"V4L2_CID_BRIGHTNESS,val=%d\n",c->value);
		break;
	case V4L2_CID_HUE:
		if (cam->sensor) {
			c->value = cam->hue;
...
log_msg(LOG_DEBUG,"V4L2_CID_HUE,val=%d\n",c->value);
		break;
...
log_msg(LOG_DEBUG,"V4L2_CID_CONTRAST,val=%d\n",c->value);
		break;
...
log_msg(LOG_DEBUG,"V4L2_CID_SATURATION,val=%d\n",c->value);
		break;
...
log_msg(LOG_DEBUG,"V4L2_CID_RED_BALANCE,val=%d\n",c->value);
		break;
...
log_msg(LOG_DEBUG,"V4L2_CID_BLUE_BALANCE,val=%d\n",c->value);
		break;
...
log_msg(LOG_DEBUG,"V4L2_CID_BLACK_LEVEL,val=%d\n",c->value);
		break;
	default:
		pr_err("ERROR: v4l2 capture: unsupported ioctrl!\n");
log_msg(LOG_DEBUG,"id=%x,val=%d\n",c->id,c->value);
	}
...
log_msg(LOG_DEBUG,"V4L2_CID_EXPOSURE,val=%d\n",c->value);
		break;
...
static int mxc_v4l2_s_param(cam_data *cam, struct v4l2_streamparm *parm)
{
	...
	if (cam->device_type != 1) {
		...
		log_msg(LOG_DEBUG,"set crop_current width %d set crop_current height %d \n", \
			cam->crop_current.width,cam->crop_current.height);
	}
	 ...
	 * size of the image. */
	log_msg(LOG_DEBUG," crop_current.width=%d  crop_current.height=%d \n", \
			cam->crop_current.width,cam->crop_current.height);
...
static int mxc_v4l_open(struct file *file)
{
	...
	if (cam->sensor == NULL ||
		...
		pr_err("ERROR: v4l2 capture: slave not found!\n");
log_msg(LOG_ERROR,"cam->sensor == %x cam=%08x\n",cam->sensor,cam);
		return -EAGAIN;
	}
			    ...
			    unsigned int ioctlnr, void *arg)
{
	...
	case VIDIOC_G_PARM: {
		...
		if (cam->sensor)
			...
			pr_err("ERROR: v4l2 capture: slave not found! vidioc_int_g_parm\n");
			retval = -ENODEV;
		}
		break;
	}
	...
	case VIDIOC_S_PARM:  {
		...
		if (cam->sensor)
			...
			pr_err("ERROR: v4l2 capture: slave not found! mxc_v4l2_s_param\n");
			retval = -ENODEV;
		}
		break;
	}
	...
	case VIDIOC_ENUM_FRAMESIZES: {
		...
		if(fsize->pixel_format==V4L2_PIX_FMT_RGB32)
		{
			log_msg(LOG_DEBUG,"subdev use fmt rgb32\n");
		}
		break;
	}
	case VIDIOC_DBG_G_CHIP_IDENT: {
		...
		log_msg(LOG_DEBUG,"chip ident name(%s)",p->match.name);
		break;
	}
	case VIDIOC_TRY_FMT:
...
log_msg(LOG_DEBUG,"slave.name = %s master.name = %s MXC_SENSOR_NUM(%d) cam=%x\n", \
	slave->name,slave->u.slave->master->name,MXC_SENSOR_NUM,cam);
log_msg(LOG_DEBUG,"cam->csi(%x),sdata->csi(%x)\n",cam->csi,sdata->csi);
	...
	pr_debug("End of %s: crop_current widthxheight %d x %d\n",
		 __func__,
		 cam->crop_current.width, cam->crop_current.height);
log_msg(LOG_DEBUG,"exit success cam->sensor=%08x,cam->sensor->name(%s)\n",cam->sensor,cam->sensor->name);
	return 0;
}
